...
def split_message(message, max_length=1800):
    ...
    for line in message.split("\n"):
        if len(current) + len(line) + 1 > max_length:
            ...
            current = line if current == "" else current + "\n" + line
    if current != "":
        parts.append(current)
    return parts
...
def fetch_market_taiex():
    ...
    while chunk_start <= end_date:
        chunk_end = min(chunk_start + timedelta(days=20), end_date)
        params = finmind_base_params()
        params.update({
...
def analyze_market():
    try:
        ...
        if len(rows) < 65:
            return {"status": "NO_ENOUGH_DATA", "date": "", "market_close": None, "market_ma60": None, "market_above_ma60": False, "market_return_5d": None}
        closes = [to_float(row.get("close")) for row in rows]
        ma60 = sma(closes, 60)
        i = len(rows) - 1
        close = closes[i]
        market_ma60 = ma60[i]
        market_above_ma60 = close is not None and market_ma60 is not None and close > market_ma60
        ...
        return {"status": "OK", "date": rows[i].get("date", ""), "market_close": close, "market_ma60": market_ma60, "market_above_ma60": market_above_ma60, "market_return_5d": market_return_5d}
    except Exception as error:
        return {"status": "ERROR", "date": "", "market_close": None, "market_ma60": None, "market_above_ma60": False, "market_return_5d": None, "error": str(error)}
...
def analyze_foreign(stock_id):
    try:
        ...
        daily_net = {}
        for row in rows:
            if str(row.get("name", "")) != "Foreign_Investor":
                continue
        ...
        if len(days) == 0:
            return {"foreign_net_latest": None, "foreign_buy_streak": 0, "foreign_buy_3d": False}
        streak = 0
        ...
        for day in reversed(days):
            if daily_net[day] > 0:
                ...
                break
        return {"foreign_net_latest": latest_net, "foreign_buy_streak": streak, "foreign_buy_3d": streak >= 3}
    except Exception:
        return {"foreign_net_latest": None, "foreign_buy_streak": 0, "foreign_buy_3d": False}
...
def analyze_stock(stock, market):
    ...
    if len(rows) < 130:
        return {"stock_id": stock_id, "name": name, "status": "NO_ENOUGH_DATA", "error": "資料不足"}
    ...
    note = "無明顯風險註記" if len(note_parts) == 0 else "；".join(note_parts)
...
def create_excel(results, market):
    ...
    append_row(conditions, [
        ...
        "market_filter", "foreign_buy_3d", "stronger_than_market", "long_upper_shadow"
    ])

    append_row(raw_latest, ["date", "stock_id", "name", "open", "high", "low", "close", "volume", "amount"])

    append_row(market_sheet, ["date", "market_close", "market_ma60", "market_above_ma60", "market_return_5d", "status"])
    ...
    for item in results:
        ...
        append_row(summary, [
            ...
            item["condition_count"], item["risk_count"], round_value(item["close"], 2),
            round_value(item["day_return"], 2), round_value(item["return_5d"], 2),
            round_value(item["market_return_5d"], 2), round_value(item["relative_strength_5d"], 2),
            round_value(item["rsi14"], 2), round_value(item["adx14"], 2), round_value(item["macd"], 4),
            round_value(item["signal"], 4), round_value(item["hist"], 4), round_value(item["ema20"], 2),
            round_value(item["ema60"], 2), round_value(item["ema120"], 2), round_value(item["volume_ratio"], 2),
            round_value(item["amount_ma20"], 0), item["foreign_net_latest"], item["foreign_buy_streak"], item["note"]
        ])

        append_row(conditions, [
            item["date"], item["stock_id"], item["name"], bool_text(item["macd_golden_cross"]),
            bool_text(item["hist_turn_positive"]), bool_text(item["above_ema60"]), bool_text(item["ema60_gt_ema120"]),
            bool_text(item["volume_break"]), bool_text(item["rsi_strong"]), bool_text(item["adx_trending"]),
            bool_text(item["breakout_20d"]), bool_text(item["avoid_chase"]), bool_text(item["liquidity_ok"]),
            bool_text(item["market_filter"]), bool_text(item["foreign_buy_3d"]), bool_text(item["stronger_than_market"]),
            bool_text(item["long_upper_shadow"])
        ])

        append_row(raw_latest, [
            item["date"], item["stock_id"], item["name"], round_value(item["open"], 2),
            round_value(item["high"], 2), round_value(item["low"], 2), round_value(item["close"], 2),
...
def build_discord_summary(results, market):
    ...
    important = sorted(important, key=lambda x: (x["grade"], -x["condition_count"], x["risk_count"]))
