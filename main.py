...
def calc_adx(highs, lows, closes, period=14):
    ...
    for i in range(len(closes)):
        ...
        if any(value is None for value in tr_window):
            dx_values.append(None)
            continue
...
def analyze_stock(stock):
    ...
    if len(rows) < 130:
        return {"stock_id": stock_id, "name": name, "status": "ERROR", "error": "資料不足或查無資料"}
    ...
    prev20_high = None
    if len(closes) >= 21:
        valid_prev20 = [value for value in closes[i - 20:i] if value is not None]
    ...
    note = "無明顯風險註記" if len(notes) == 0 else "；".join(notes)
...
def create_excel(results):
    ...
    summary.append([
        ...
        "close", "day_return_pct", "return_5d_pct", "rsi14", "adx14",
        "macd", "signal", "hist", "ema20", "ema60", "ema120",
    ...
    conditions.append([
        ...
        "adx_trending", "breakout_20d", "avoid_chase", "liquidity_ok", "long_upper_shadow"
    ])

    raw_latest.append([
        "date", "stock_id", "name", "open", "high", "low", "close", "volume", "amount"
    ])
    ...
    for item in results:
        ...
        summary.append([
            ...
            item["condition_count"], item["risk_count"], safe_round(item["close"], 2),
            safe_round(item["day_return"], 2), safe_round(item["return_5d"], 2),
            safe_round(item["rsi14"], 2), safe_round(item["adx14"], 2),
            safe_round(item["macd"], 4), safe_round(item["signal"], 4),
            safe_round(item["hist"], 4), safe_round(item["ema20"], 2),
            safe_round(item["ema60"], 2), safe_round(item["ema120"], 2),
            safe_round(item["volume_ratio"], 2), safe_round(item["amount_ma20"], 0),
            item["note"]
        ])

        conditions.append([
            item["date"], item["stock_id"], item["name"],
            bool_text(item["macd_golden_cross"]), bool_text(item["hist_turn_positive"]),
            bool_text(item["above_ema60"]), bool_text(item["ema60_gt_ema120"]),
            bool_text(item["volume_break"]), bool_text(item["rsi_strong"]),
            bool_text(item["adx_trending"]), bool_text(item["breakout_20d"]),
            bool_text(item["avoid_chase"]), bool_text(item["liquidity_ok"]),
            bool_text(item["long_upper_shadow"])
        ])

        raw_latest.append([
            item["date"], item["stock_id"], item["name"], safe_round(item["open"], 2),
            safe_round(item["high"], 2), safe_round(item["low"], 2),
            safe_round(item["close"], 2), item["volume"], item["amount"]
        ])
