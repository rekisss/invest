...
const RULES = [
  ...
  '鐵則四：若資料稽核員宣告「嚴重不足」，所有角色直接輸出資料清單，不得進入正式分析。',
]
...
【資料稽核結果】
逐項列出：有值的 → 覆述關鍵數字；缺的 → 標「缺」並點名欄位。

【資料充足度判定】
從以下三擇一：
• 足夠：股票代號、資料日期、最新收盤價、成交量均存在
• 技術位不足：基本欄位存在但缺 MA/RSI/ADX/法人資料
• 嚴重不足：缺少收盤價或成交量等核心欄位

【給後續發言者的提醒】
若判定「嚴重不足」→ 請所有角色直接列出所需資料清單，不得進入正式分析，不得給出任何建議或數字。
